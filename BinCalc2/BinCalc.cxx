...
void Count(Fl_Button* arr[], Fl_Input* who)
{
	...
	for (int i = 0; i < bitArr[bit]; i++)
		...
		case 0: 
			a += arr[i]->label() == "1" ? slider << (bitArr[bit] - 1 - i) : 0; 
			break;
		case 2:
			b += arr[i]->label() == "1" ? slider << (bitArr[bit] - 1 - i) : 0; 
			break;
		case 4:
			c += arr[i]->label() == "1" ? slider << (bitArr[bit] - 1 - i) : 0; 
			break;
		case 6:
			d += arr[i]->label() == "1" ? slider << (bitArr[bit] - 1 - i) : 0; 
			break;
...
void ChangeBit()
{
	...
	{
	case 0:
		bitChanger->label("8"); break;
	case 2:
		bitChanger->label("16"); break;
	case 4:
		bitChanger->label("32"); break;
	case 6:
		bitChanger->label("64"); break;
...
void SetValue(Fl_Input* who, Fl_Button* arr[])
{
	...
	{
	case 10: 
		sscanf(who->value(), "%d", &val); break;
	case 16:
		sscanf(who->value(), "%x", &val); break;
	case 8:
...
void SetValue(Fl_Input* who, Fl_Button* arr[], bool)
{
	...
	{
	case 10: 
		sscanf(who->value(), "%lld", &val); break;
	case 16:
		sscanf(who->value(), "%x", &val); break;
	case 8: 
		sscanf(who->value(), "%o", &val); break;
...
void DoOperation(Fl_Button* but, void*)
{
	...
	{
	case 0: 
		RePrint((char)ans, inputC);
		SetValue(inputC, C[bitArr[bit + 1]]); break;
	case 2: 
		RePrint((short)ans, inputC);
		SetValue(inputC, C[bitArr[bit + 1]]); break;
	case 4: 
		RePrint((int)ans, inputC); 
		SetValue(inputC, C[bitArr[bit + 1]]); break;
	case 6:
		RePrint(ans, inputC, true); 
		SetValue(inputC, C[bitArr[bit + 1]], true); break;
...
void GetInput(Fl_Input* inp)
{
	...
	{
	case aiY:
		SetValue(inputA, A[bitArr[bit + 1]]); break;
	case biY:
		SetValue(inputB, B[bitArr[bit + 1]]); break;
	case ciY:
		RePrint(cC, inputC); break;
	}
}
